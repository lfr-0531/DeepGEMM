...
void sm100_fp8_paged_mqa_logits(const uint32_t batch_size,
    ...
    constexpr uint32_t kComputeBlockKV = 128;
    ...
    DG_STATIC_ASSERT(kNumMathThreads == 256 and kNumSpecializedThreads == 128, "Invalid kNumMathThreads or kNumSpecializedThreads");
    // The following warp configuration is based on the assumption that "kNumMathThreads == 256 and kNumSpecializedThreads == 128"
    const bool& is_math_warp = (warp_idx < (kNumMathThreads / 32));
    const bool& is_tma_load_warp = (warp_idx == kNumMathThreads / 32) or (warp_idx == kNumMathThreads / 32 + 1);
    const bool& is_umma_warp = (warp_idx == kNumMathThreads / 32 + 2) or (warp_idx == kNumMathThreads / 32 + 3);

    // Separate math warpgroups and tma load warps into KV groups
    // Each math warpgroup corresponds to a tma load warp
    auto kv_group_idx = __shfl_sync(0xffffffff, threadIdx.x >= kNumMathThreads ? (threadIdx.x - kNumMathThreads) / 32 : warpgroup_idx, 0);
    if (is_umma_warp) {
        kv_group_idx %= 2;
    }
    ...
    if (is_umma_warp) {
        if (cute::elect_one_sync()) {
            full_umma_barriers[kv_group_idx]->init(1);
            empty_umma_barriers[kv_group_idx]->init(128);
            cutlass::arch::fence_barrier_init();
        }
    ...
    constexpr uint32_t UMMA_M = kComputeBlockKV;
    ...
    if (is_tma_load_warp) {
        ...
        const auto& issue_tma_q = [&](const uint32_t& stage_idx, const uint32_t& q_idx) {
            if (kv_group_idx == 0 and cute::elect_one_sync()) {
                tma_copy<kHeadDim, kNextNPerCTA * kNumHeads, kHeadDim>(&tensor_map_q, full_q_barriers[stage_idx], smem_q[stage_idx], 0, (q_idx * kNextN + cta_rank_in_cluster * kNextNPerCTA) * kNumHeads);
                tma_copy<kNextNPerCTA * kNumHeads, 1, 0>(&tensor_map_weights, full_q_barriers[stage_idx], smem_weights[stage_idx], cta_rank_in_cluster * kNextNPerCTA * kNumHeads, q_idx);
        ...
        while (scheduler.fetch_next_task(next_q_idx, next_kv_idx, next_num_kv)) {
            ...
            for (uint32_t k = 0; k < kHeadDim / UMMA_K; ++ k) {
                auto a_desc = make_umma_desc<cute::UMMA::Major::K, 0, kHeadDim, kHeadDim>(
                    smem_kv[kv_stage_idx] + kv_group_idx * SMEM_KV_PIPE_SIZE, 0, k * UMMA_K);
                auto b_desc = make_umma_desc<cute::UMMA::Major::K, 0, kHeadDim, kHeadDim>(
                    smem_q[q_stage_idx], 0, k * UMMA_K);
                cute::SM100_MMA_F8F6F4_SS::fma(a_desc, b_desc, kv_group_idx * UMMA_N, k, runtime_instr_desc);
            }
            cutlass::arch::umma_arrive(reinterpret_cast<uint64_t*>(full_umma_barriers[kv_group_idx]));
            umma_phase ^= 1;
        }
        ...
        const auto& warp_offset = (warp_idx % 4) * 32;
        ...
        while (scheduler.fetch_next_task(next_q_idx, next_kv_idx, next_num_kv)) {
            ...
            auto kv_offset = (q_idx * kNextN + cta_rank_in_cluster * kNextNPerCTA) * logits_stride + (kv_idx + kv_group_idx) * kComputeBlockKV + warp_offset;
            ...
            constexpr uint32_t kNumLDTMElems = kNumHeads * kNextNPerCTA;
            ...
            for (uint32_t i = 0; i < kNextNPerCTA; ++ i) {
                ...
                logits[kv_offset + i * logits_stride + v_offset] = result;
            }
        }
    } else {
        cutlass::arch::warpgroup_reg_dealloc<kNumSpecializedRegisters>();

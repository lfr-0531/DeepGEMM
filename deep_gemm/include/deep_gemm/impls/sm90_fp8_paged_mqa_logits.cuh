...
void sm90_fp8_paged_mqa_logits(const uint32_t batch_size,
    ...
    // Scheduler
    auto scheduler = sched::PagedMQALogitsScheduler<kNextN, kIsContextLens2D, kComputeBlockKV, kNumMathWarpGroups, 1>(
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        while (fetched_next_task) {
            ...
            if (kv_idx == 0 or kv_block_idx_ptr == 32) {
                ...
                kv_block_idx_storage = (kv_idx + kv_group_idx + lane_idx * kNumMathWarpGroups < num_kv ?
                    __ldg(reinterpret_cast<const idx_storage_t*>(block_table) + q_idx * static_cast<uint64_t>(block_table_stride) / kNumBlocksPerMMA
        ...
        while (scheduler.fetch_next_task(next_q_idx, next_kv_idx, next_num_kv)) {
            ...
            auto kv_offset = q_idx * kNextN * static_cast<uint64_t>(logits_stride) + ((kv_idx + kv_group_idx) * kComputeBlockKV + sub_warp_offset);

            // Compute `[kNextN * kNumHeads, kHeadDim] @ [kComputeBlockKV, kHeadDim] -> [kNextN, kComputeBlockKV]`
            // Wait TMA KV arrival
            CUTE_TIE_DECL(get_kv_pipeline(kv_iter_idx ++), kv_stage_idx, kv_phase);

...
static torch::Tensor fp8_fp4_mqa_logits(const std::tuple<torch::Tensor, std::optional<torch::Tensor>>& q,
    ...
    // SM120 tiles 128 KV rows per block; SM90/SM100 tile 256
    ...
    // NOTES: the logits cleaning is fused into the SM100/SM90 kernels; the FP16-weights
    // and SM120 kernels do not fuse it and run the standalone cleaner afterwards
    if (weights_is_f16) {
        ...
        // Clean unfilled logits (not fused into the SM120 kernels)
        if (clean_logits)
            smxx_clean_logits(logits, cu_seq_len_k_start, cu_seq_len_k_end, 1, seq_len, seq_len_kv, stride_logits);
    } else {
